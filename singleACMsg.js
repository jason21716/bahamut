...
function checkReplyFix(a, b) {
    var c = document.getElementById("replyMsg" + a),
        d = c.value;
    if (diceProccess(c.value))
        return;
    c.value = prePostProccess(c.value);
...
function timerControl(str, limitTime, responseText) {
    switch (str) {
        case 'start':
            ...
            BAHA_TIMER_ID = window.setInterval(function() {
                ...
                if (BAHA_TIMER_LIMIT_TIME != -1 && elapsed > BAHA_TIMER_LIMIT_TIME) {
                    timerControl('stop');
                    document.getElementById('replyMsg' + configArr['MsgId']).value = BAHA_TIMER_TEXT;
