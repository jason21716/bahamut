...
function diceProccess(str) {
    str_after = str.replace(/\[\[\s*(\d*)\s*d\s*(\d*)\s*([+-]*)\s*(\d*)\s*\(*([^\)]*)\)*\s*\]\]/i, filterRollDice);
    return (str_after !== str)
}

function prePostProccess(str) {
    str = str.replace(/\[\[\s*t\s*(\d*)\s*([^\]]*)\s*\]\]/i, filterTimer);
    return str;
}

function filterRollDice(match, var_count, var_size, var_symbol, var_addnum, var_reason, offset, string) {
    var addnumber = (!isNaN(parseInt(var_addnum))) ? parseInt(var_addnum) : 0;
    addnumber *= (var_symbol === '-') ? -1 : 1;

    var dice = {
        user: configArr['controller'],
        nickname: configArr['controller'],
    ...
    $.ajax({
        type: "POST",
        url: "http://www.isaka.idv.tw/dice-api/dice",
        dataType: 'json',
        data: dice,
        success: function(bb) {
            ...
            document.getElementById('replyMsg' + configArr['MsgId']).value = str;
            checkReplyFix(configArr['MsgId'], '#GID' + configArr['guildId'])
        }
    });
    return '';
}

function filterTimer(match, var_time, var_reason, offset, string) {
    timerControl('start', var_time, var_reason);
    return '(' + var_time + '秒判定)';
}